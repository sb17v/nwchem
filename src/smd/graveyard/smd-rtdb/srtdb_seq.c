...
int srtdb_seq_open(const char *filename, const char *mode, int *handle)
/*
...
*/
{
  ...
  for (new=0; new<MAX_RTDB; new++)
	...
	(void) fprintf(stderr, "srtdb_seq_open: %s is already open\n", 
		       filename);
	return 0;
      }
  ...
  if (strcmp(mode, "new") == 0) {
    if (exists) {
      (void) fprintf(stderr, 
		     "srtdb_seq_open: %s exists but is being opened new\n",
		     filename);
      return 0;
    }
  }
    ...
    if (!exists) {
      (void) fprintf(stderr, 
		     "srtdb_seq_open: %s does not exist, cannot open old\n",
		     filename);
      return 0;
    }
  }
    ...
    (void) fprintf(stderr, "srtdb_seq_open: unknown mode=%s\n", mode);
    return 0;
  }
  ...
  for (new=0; new<MAX_RTDB && rtdb[new].active; new++)
    ;
  if (new == MAX_RTDB) {
    (void) fprintf(stderr, "srtdb_seq_open: too many data bases open, max=%d\n",
		   MAX_RTDB);
    return 0;
  }
  ...
  if (!(rtdb[new].db = 
    ...
    (void) fprintf(stderr, "srtdb_seq_open: db failed to open file %s\n",
		   filename);
    return 0;
  }
#endif
  ...
  if (!hdbm_open(filename, 0, &rtdb[new].db)) {
    (void) fprintf(stderr, "srtdb_seq_open: hdbm failed to open file %s\n",
		   filename);
    return 0;
  }
#endif
...
int srtdb_seq_first(const int handle, const int namelen, char *name)
/*
...
*/
{
  ...
  if (status) {
    if (namelen >= SIZE(key)) {
      ...
      (void) fprintf(stderr, 
		     "srtdb_seq_first: name is too small, need=%u got=%d\n",
...
int srtdb_seq_delete(const int handle, const char *name)
/*
...
*/
{
  ...
  if (strlen(name)+sizeof(info_header)+1 > sizeof(info_buf)) {
    (void) fprintf(stderr,
		   "srtdb_delete: info entry buffer needs to be %d\n",
  ...
  if (status == 0)
    ...
    (void) fprintf(stderr, "srtdb_seq_delete: error deleting %s\n", name);
    return 0;
  }
}
    
int srtdb_seq_next(const int handle, const int namelen, char *name)
/*
...
*/
{
  ...
  if (status) {
    if (namelen >= SIZE(key)) {
      ...
      (void) fprintf(stderr, 
		     "srtdb_seq_next: name too small, need=%u, got=%d\n",
		     (int) SIZE(key), (int) namelen);
      status = 0;
    }
#ifdef USE_HDBM
...
#include "global.h"
int srtdb_seq_close(const int handle, const char *mode)
/*
  Close the data base
...
void srtdb_ma_print(FILE *file, const int ma_type, const int nelem, void *p)
{
  ...
  case MT_C_INT0:	/* Fortran integer ... not equivalent on KSR */
    for (nprint=i=0; i<nelem; i++) {
  ...
  case MT_C_INT3:	/* Fortran double precision */
  ...
  case MT_C_INT4:	/* Fortran single precision complex */
  ...
  case MT_C_INT5:	/* Fortran double precision complex */
  ...
  case MT_C_INT1:	/* Fortran logical */
    for (nprint=i=0; i<nelem; i++) {
...
int srtdb_seq_print(const int handle, const int print_values)
/*
...
*/
{
  ...
  if (!check_handle(handle)) {
    (void) fprintf(stderr, "srtdb_seq_print: handle (%d) is invalid\n", handle);
    return 0;
  }
  ...
  status = srtdb_seq_first(handle, sizeof(name), name);
  ...
  while (status) {
    ...
    if (!srtdb_seq_get_info(handle, name, &ma_type, &nelem, date)) {
      (void) fprintf(stderr, "srtdb_seq_print: get_info failed on %s\n", name);
    }
	...
	if (!srtdb_seq_ma_get(handle, name, &ma_type, &nelem, &ma_handle)) {
	  (void) fprintf(stderr, "srtdb_seq_print: ma_get failed on %s\n", name);
	}
	else {
	  void *data;
    ...
    status = srtdb_seq_next(handle, sizeof(name), name);
  }
  printf("\n");
...
static int srtdb_seq_put_info(const int handle,
...
*/  
{
  ...
  if (!check_handle(handle)) {
    (void) fprintf(stderr, "srtdb_seq_put_info: handle (%d) is invalid\n", handle);
    return 0;
  }
  ...
  if (strlen(name)+sizeof(info_header)+1 > sizeof(info_buf)) {
    (void) fprintf(stderr,
		   "srtdb_seq_put_info: info entry buffer needs to be %d\n",
		   (int) (strlen(name)+sizeof(info_header)+1));
    return 0;
  }
  ...
  if (!status) {
    (void) fprintf(stderr, 
		   "srtdb_seq_put_info: put failed for \"%s\" in %s\n",
...
int srtdb_seq_get_info(const int handle,
...
*/  
{
  ...
  if (!check_handle(handle)) {
    (void) fprintf(stderr, "srtdb_seq_get_info: handle (%d) is invalid\n", 
		   handle);
    return 0;
  }
  ...
  if (strlen(name)+sizeof(info_header)+1 > sizeof(info_buf)) {
    (void) fprintf(stderr,
		   "srtdb_seq_get_info: info entry buffer needs to be %d\n",
		   (int) (strlen(name)+sizeof(info_header)+1));
    return 0;
  }
  ...
  if (status == -1) {
    (void) fprintf(stderr, 
		   "srtdb_seq_get_info: db get failed for \"%s\" in %s\n",
		   name, rtdb[handle].filename);
  }
  status = !status;
#endif
  ...
  if (SIZE(value) != sizeof(info)) {
    (void) fprintf(stderr, 
		   "srtdb_seq_get_info: size mismatch : info=%d, db=%d\n",
...
int srtdb_seq_put(const int handle, const char *name, const int ma_type,
...
*/  
{
  ...
  if (!check_handle(handle)) {
    (void) fprintf(stderr, "srtdb_seq_put: handle (%d) is invalid\n", handle);
    return 0;
  }
  ...
  if (!status) {
    (void) fprintf(stderr, "srtdb_seq_put: put failed for \"%s\" in %s\n",
		   name, rtdb[handle].filename);
    return 0;
  }
  ...
  if (!srtdb_seq_put_info(handle, name, ma_type, nelem))
    return 0;
...
//  status = hdbm_file_flush(db);  
  ...
  if (!status) {
    (void) fprintf(stderr, "srtdb_seq_put: flush failed for \"%s\" in %s\n",
		   name, rtdb[handle].filename);
    return 0;
  }
...
int srtdb_seq_get(const int handle, const char *name, const int ma_type,
...
*/
{
  ...
  if (!check_handle(handle)) {
    (void) fprintf(stderr, "srtdb_seq_get: handle (%d) is invalid\n", handle);
    return 0;
  }

  /* Retrieve the info from the data base to check types etc. */

  if (!srtdb_seq_get_info(handle, name, &ma_type_from_db, &nelem_from_db, date)) {

    /* In production will want to have this handled quietly  ... be
       verbose for debug only 

    (void) fprintf(stderr, "srtdb_seq_get: get info failed for \"%s\" in %s\n",
		   name, rtdb[handle].filename); */
    return 0;
  }
  ...
  if (ma_type_from_db != ma_type) {
    (void) fprintf(stderr, 
		   "srtdb_seq_get: type mismatch \"%s\" in %s: arg=%d, db=%d\n",
  ...
  if (!status) {
    (void) fprintf(stderr, "srtdb_seq_get: get failed for \"%s\" in %s\n",
		   name, rtdb[handle].filename);
    return 0;
  }
  ...
  if (nelem_actual != nelem_from_db) {
    (void) fprintf(stderr, 
		   "srtdb_seq_get: size error \"%s\" in %s: info=%d, db=%d\n",
  ...
  if (nelem_actual > nelem) {
    (void) fprintf(stderr, "srtdb_seq_get: \"%s\" is %d long, buffer is %d\n",
...
int srtdb_seq_ma_get(const int handle, const char *name, int *ma_type,
...
*/
{
  ...
  if (!check_handle(handle)) {
    (void) fprintf(stderr, "srtdb_seq_ma_get: handle (%d) invalid\n", handle);
    return 0;
  }
  ...
  if (!srtdb_seq_get_info(handle, name, ma_type, nelem, date)) {
    /*
    (void) fprintf(stderr, "srtdb_seq_ma_get: get info failed \"%s\" in %s\n",
		   name, rtdb[handle].filename);
                   */
    return 0;
  }
  ...
  if (!MA_allocate_heap(ma_type_buf, nelem_buf, name, &ma_handle_buf)) {
    (void) fprintf(stderr, "srtdb_seq_ma_get: MA_allocate_heap : nelem=%d\n",
		   *nelem);
    return 0;
  }
  *ma_handle = ma_handle_buf;
  ...
  if (!MA_get_pointer(ma_handle_buf, &ma_data)) {
    (void) fprintf(stderr, "srtdb_seq_ma_get: MA_get_pointer failed\n");
    return 0;
  }

  if (!srtdb_seq_get(handle, name, *ma_type, *nelem, ma_data)) {
    (void) fprintf(stderr, "srtdb_seq_ma_get: srtdb_seq_get failed %s\n", name);
...
int srtdb_seq_copy(const int handle, const char *suffix)
/*
  Copy the data base

  handle  = handle to RTDB
  suffix    = new file will be called srtdb_name.suffix
*/
{
	...
	(void) fprintf(stderr,
		       "srtdb_seq_copy: copy from %s failed\n", suffix);
      return 0;
    }
#else
fixme
...
int srtdb_seq_getfname(const int handle,
		  char fname[36])
/*
  Get rtdb file name
